...
def capture_images(image_task, sleep_between_capture, RAW_IMAGES_QUEUE):
...
def process_images(model_path, detection_class, detection_threshold, max_processing_delay, RAW_IMAGES_QUEUE, PROCESSED_BOXES_QUEUE):
...
def get_go_to(world_tform_object, robot_state, mobility_params, dist_margin=1):
...
def get_mobility_params():
    ...
    mobility_params = spot_command_pb2.MobilityParams(vel_limit=speed_limit, obstacle_params=obstacles,
...
def main(argv):
    ...
    try:
        ...
        print("Starting tensorflow process..")
        ...
        process = Process(target=process_images, args=(
            options.model_path,
            options.detection_class,
            options.detection_threshold,
            options.max_processing_delay,
            RAW_IMAGES_QUEUE, PROCESSED_BOXES_QUEUE,
        ))
        process.start()
        ...
        image_capture_thread = Thread(target=capture_images,
                                      args=(image_task, options.sleep_between_capture, RAW_IMAGES_QUEUE))
