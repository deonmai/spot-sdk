...
def process_images(index, model_path, detection_classes, detection_threshold, max_processing_delay, RAW_IMAGES_QUEUE, PROCESSED_IMAGES_QUEUE):
...
def display_images(max_display_delay, PROCESSED_IMAGES_QUEUE):
...
class SpotImageCapture:
    ...
    def capture_images(self, sleep_between_capture, RAW_IMAGES_QUEUE):
...
def main(argv):
    ...
    try:
        ...
        process = Process(target=process_images, args=(
            ...
            options.max_processing_delay,
            RAW_IMAGES_QUEUE, PROCESSED_IMAGES_QUEUE,
        ))
        process.start()

        # Start Display process
        print("Starting display process..")
        display = Process(target=display_images, args=(options.max_display_delay, PROCESSED_IMAGES_QUEUE))
        display.start()
        ...
        while True:
            image_capture.capture_images(options.sleep_between_capture, RAW_IMAGES_QUEUE)
